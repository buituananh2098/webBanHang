  ...
  <script type="text/javascript">
    $('.trash').click(function() {
      if (confirm('Bạn có muốn xóa?')) {
        window.location.href = '{{ route('admin.role.destroy') }}'+'/'+$(this).attr('data-trash');
      }
    });
  </script>
@endsection
...
<div class="padding">
  <div class="mb-2">
    <div class="col-12 text-right">
      <a href="{{ route('admin.role.create') }}"><input type="button" class="btn btn-raised btn-wave blue text-white btn-sm" value="Thêm mới"></a>
    </div>
  </div>
  <div class="mb-5">
    <div class="toolbar">
      ...
      <table class="table table-theme table-row v-middle">
        <thead>
          <tr>
            ...
            <th class="text-muted">Tên vai trò</th>
        ...
        <tbody>
          ...
          <tr class="v-middle">
            ...
            <td>{{ $role->id }}</td>
            <td>{{ $role->name }}</td>
            <td><span class="item-amount  text-xl-left {{ ($role->active==1) ? 'text-success' : 'text-danger' }}">{{ ($role->active==1) ? 'Hoạt động' : 'Khóa' }}</span></td>
            <td>
            <td>
              <div class="item-action dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right bg-black" role="menu" x-placement="bottom-end" style="position: absolute; top: 0px; left: 0px;">
                  <a href="{{ route('admin.role.edit', $role->id) }}" class="dropdown-item edit">Sửa</a>
                  <a href="javascript:void()" class="dropdown-item trash" data-trash="{{ $role->id }}">Xóa</a>
                </div>
              </div>
            </td>
          </tr>
