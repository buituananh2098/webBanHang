    ...
    <script type="text/javascript">
        $('.trash').click(function () {
            if (confirm('Bạn có muốn xóa?')) {
                window.location.href = '{{ route('admin.product.destroy') }}' + '/' + $(this).attr('data-trash');
            }
        });
    </script>
@endsection
    ...
    <div class="padding">
        <div class="mb-2">
            <div class="col-12 text-right">
                <a href="{{ route('admin.product.create') }}"><input type="button"
                ...
                <table class="table table-theme table-row v-middle">
                    ...
                    @foreach ($products as $product)
                        <tr class="v-middle">
                            ...
                            <td>{{ $product->id }}</td>
                            <td width="100px">
                                <img class="img-avt" src="{{ !empty($product->image[0]->path) ? $product->image[0]->path : '/images/img_err.png' }}" alt="{{ $product->name }}">
                            </td>
                            <td>{{ $product->name }}</td>
                            <td>{{ $product->code }}</td>
                                ...
                                <div class="item-action dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right bg-black" role="menu"
                                        ...
                                        <a href="{{ route('admin.product.destroy', $product->id) }}"
                                           class="dropdown-item trash" data-trash="{{ $product->id }}">Xóa</a>
                                    </div>
                                </div>
                            </td>
                        </tr>
